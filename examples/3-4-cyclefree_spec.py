...
def solve(total_vertex_count: int) -> set[tuple[int, int]]:
  ...
  vertex_to_neighbors = {
    v: set(
      w
      for w in range(total_vertex_count)
      if (graph[v, w] == 1 or graph[w, v] == 1) and v != w
    )
    for v in range(total_vertex_count)
  }

  for vertex_v, vertex_w in [
    (v, w) for v in range(total_vertex_count) for w in vertex_to_neighbors[v] if w > v
  ]:
    ...
    for neighbor_u in vertex_to_neighbors[vertex_v]:
      if neighbor_u != vertex_w and not vertex_to_neighbors[neighbor_u].isdisjoint(
        vertex_to_neighbors[vertex_w]
      ):
        vertex_to_neighbors[vertex_v].remove(vertex_w)
        vertex_to_neighbors[vertex_w].remove(vertex_v)
        break
