...
if __name__ == "__main__":
    # Map from config-string to (name, digested_islands)
    ...
    for backup in Path("data/backups").glob("*.pickle"):
        ...
        identities[config_str].append((backup.name, digested_islands))
    ...
    for items in identities.values():
        non_subsets = []
        for ix_a, (a_name, a_digest) in enumerate(items):
            was_subset = False
            for b_name, b_digest in items[ix_a + 1 :]:
                if sub_digestion(a_digest, b_digest):
                    print(f"{a_name} is a subset of {b_name}")
                    was_subset = True
                    break
            if not was_subset:
                non_subsets.append(a_name)
        if len(non_subsets) > 1:
            print(f"All have the same config, but are not subsets of each other: {non_subsets}")
