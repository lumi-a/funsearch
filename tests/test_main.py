...
def test_cli(tmp_path: Path):
    ...
    assert database._config.functions_per_prompt == functions_per_prompt, (
        "Functions per prompt was not saved in Database"
    )
    ...
    assert sum(len(island._runs) - 1 for island in resume_database._islands) == 2 * samples, (
